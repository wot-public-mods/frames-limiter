...
namespace frames_limiter {
	...
	FramesLimiter::FramesLimiter() : _hooks_d3d(OpenWG::Native::Hooks::HookmanagerD3D::instance())
	{
		set_target_fps(0);
	...
	bool FramesLimiter::dxgi_hook()
	{
		...
		if (_hooks_d3d.inited()) {
			_hooks_d3d.IDXGISwapChain_Present_register("poliroid_frames_limiter", OpenWG::Native::Hooks::HookPlace::Before,
	...
	bool FramesLimiter::dxgi_unhook()
	{
		...
		if (_hooks_d3d.inited()) {
			_hooks_d3d.IDXGISwapChain_Present_unregister("poliroid_frames_limiter", OpenWG::Native::Hooks::HookPlace::Before);
			result = true;
		}
